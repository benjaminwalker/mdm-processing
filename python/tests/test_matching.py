...
from mdm_processing.core.matching import AmbiguousMatchError, CrossChannelOutcome, match_entity
...
def test_no_match_creates_new_master():
    ...
    result = match_entity(repo, ENTITY_CONFIG, CHANNEL, incoming, NOW)
...
def test_matches_existing_master_by_natural_key():
    repo = InMemoryMasteryRepository()
    first = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("1", {"ssn": "123-45-6789"}), NOW)

    second = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("2", {"ssn": "123-45-6789"}), NOW)
...
def test_matches_via_second_natural_key_when_first_absent():
    repo = InMemoryMasteryRepository()
    first = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("1", {"ssn": "123-45-6789", "email": "a@example.com"}), NOW)

    second = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("2", {"email": "a@example.com"}), NOW)
...
def test_missing_natural_key_values_are_skipped_not_matched():
    repo = InMemoryMasteryRepository()
    match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("1", {"ssn": "123-45-6789"}), NOW)

    result = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("2", {"name": "no natural keys here"}), NOW)
...
def test_conflicting_natural_keys_raise_ambiguous_match():
    repo = InMemoryMasteryRepository()
    master_a = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("1", {"ssn": "111-11-1111"}), NOW)
    master_b = match_entity(repo, ENTITY_CONFIG, CHANNEL, _incoming("2", {"email": "b@example.com"}), NOW)
    assert master_a.master_key != master_b.master_key

    conflicting = _incoming("3", {"ssn": "111-11-1111", "email": "b@example.com"})
    with pytest.raises(AmbiguousMatchError) as exc_info:
        match_entity(repo, ENTITY_CONFIG, CHANNEL, conflicting, NOW)

    assert set(exc_info.value.master_keys) == {master_a.master_key, master_b.master_key}


def test_without_scorer_probabilistic_fallback_is_skipped_and_master_is_created():
    repo = InMemoryMasteryRepository()
    match_entity(
    ...
    result = match_entity(repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL, incoming, NOW)  # no scorer passed

    assert result.outcome == CrossChannelOutcome.MASTER_CREATED


def test_probabilistic_fallback_matches_highest_scorer_above_threshold():
    repo = InMemoryMasteryRepository()
    full_match = match_entity(
        repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL,
        _incoming("1", {"ssn": "111-11-1111", "name": "Alice Smith", "date_of_birth": "1990-01-01"}), NOW,
    )
    match_entity(
    ...
    result = match_entity(repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL, incoming, NOW, _fraction_match_scorer)

    assert result.outcome == CrossChannelOutcome.MASTER_MATCHED_PROBABILISTIC
    assert result.master_key == full_match.master_key


def test_probabilistic_fallback_below_threshold_creates_new_master():
    repo = InMemoryMasteryRepository()
    match_entity(
        repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL,
...
def test_probabilistic_fallback_tied_top_scores_above_threshold_raise_ambiguous():
    repo = InMemoryMasteryRepository()
    master_a = match_entity(
        repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL,
        _incoming("1", {"ssn": "111-11-1111", "name": "Alice Smith", "date_of_birth": "1990-01-01"}), NOW,
    )
    master_b = match_entity(
        repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL,
    ...
    incoming = _incoming("3", {"name": "Alice Smith", "date_of_birth": "1990-01-01"})
    with pytest.raises(AmbiguousMatchError) as exc_info:
        match_entity(repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL, incoming, NOW, _fraction_match_scorer)

    assert set(exc_info.value.master_keys) == {master_a.master_key, master_b.master_key}


def test_deterministic_match_takes_priority_over_probabilistic():
    repo = InMemoryMasteryRepository()
    deterministic_target = match_entity(
        repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL,
        _incoming("1", {"ssn": "111-11-1111", "name": "Alice Smith", "date_of_birth": "1990-01-01"}), NOW,
    )

    # Would also score a perfect probabilistic match against itself, but the natural key match should win outright.
    incoming = _incoming("2", {"ssn": "111-11-1111", "name": "Alice Smith", "date_of_birth": "1990-01-01"})
    result = match_entity(repo, PROBABILISTIC_ENTITY_CONFIG, CHANNEL, incoming, NOW, _fraction_match_scorer)

    assert result.outcome == CrossChannelOutcome.MASTER_MATCHED_DETERMINISTIC
    assert result.master_key == deterministic_target.master_key
